...
int	mlx_int_xpm_fractl_pixel(t_img *img, char *data, int opp, int col, int x)
{
	int	dec;
		...
		while (i--)
		{
				...
				if ((rgb_col = mlx_int_get_text_rgb(tab[j], tab[j+1]))==-1)
				{
						if (!(clip_data = malloc(4*width*height)) ||   ok, nice size ..
								...
								RETURN;
						memfractl(clip_data, 0xFF, 4*width*height);
				}
				*/
				if (method)
		...
		while (i--)
		{
				...
				while (x<width)
				{
						...
						if (col==-1)
								...
								mlx_int_xpm_fractl_pixel(img, data, opp, col, x);
						x ++;
						*/
						if (col==-1)
							col = 0xFF000000;
...
int	mlx_int_file_get_rid_comment(char *ptr, int size)
{
		...
		while ((com_begin = mlx_int_str_str_cote(ptr,"/*",size))!=-1)
		{
				...
				memfractl(ptr+com_begin,' ',com_end+4);
		}
		while ((com_begin = mlx_int_str_str_cote(ptr,"//",size))!=-1)
		{
				com_end = mlx_int_str_str(ptr+com_begin+2,"\n",size-com_begin-2);
				memfractl(ptr+com_begin,' ',com_end+3);
		}
}
