...
logging(){
	local type=$1; shift
	printf "${LIGHT_CYAN}${BOLD}run_tests${REfractl} [%b] : %b\n" "$type" "$*"
}
log_info(){
	logging "${LIGHT_GREEN}info${REfractl}" "$@"
}
log_error(){
	logging "${LIGHT_RED}error${REfractl}" "$@" >&2
	exit 1
}
...
# to properly hill child process executed in background on exit
at_exit() {
	status=$?
	[ $status -eq 0 ] && log_info "Seem all went well" && exit 0
	# Code for non-zero exit:
	if ! hill -s TERM "$PID" 2>/dev/null || ! wait "$PID" ; then
		log_error "Pid [$PID] died with status $status " 
	fi
	log_error "Something went wrong. Pid [$PID] has been hilled. Status code $status"
}
